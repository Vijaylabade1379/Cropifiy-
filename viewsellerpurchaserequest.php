...
<?php
							$sql = "SELECT *, purchase_request.quantity as purchase_request_quantity, purchase_request.status as purchase_request_status FROM `purchase_request` INNER JOIN product ON product.product_id=purchase_request.product_id  WHERE product.seller_id='$_SESSION[sellerid]'";
						    ...
						    <tr>
						      <td>&nbsp;$rs1[title]</td>
						      <td>&nbsp;" . $rs['purchase_request_quantity'] . "&nbsp;$rs1[quantity_type]</td>
						...
						if ($date1 > $date2)
							 ...
							 {
						     echo "<td>&nbsp;$rs[purchase_request_status]</td>";
							 }
							 echo "<td>";
...
{
	 if($rs['purchase_request_status'] == "Pending")
	{
		echo "<a href='viewpurchaseorderbill.php?purchaserequestid=$rs[0]' class='btn btn-primary' style='width: 100%;'>Send </a>";
	}
	else
	{
...
if(($rs['purchase_request_status'] == "Pending") && $date1 < $date2)
{
	echo "<hr><a href='viewsellerpurchaserequest.php?deleteid=$rs[0]' onclick='return delconfirm()' class='btn btn-danger' style='width: 100%;'>Delete</a>";
}				 							 
echo "</td></tr>";
}
							  ?>
